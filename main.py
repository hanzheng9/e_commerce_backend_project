from fastapi import (FastAPI, BackgroundTasks, Request, HTTPException, status, Depends)
from tortoise import models
from tortoise.transactions import in_transaction
from tortoise.contrib.fastapi import register_tortoise
from models import *
#from models import user_pydanticIn
...
'''async def add_date_published_column():
    async with in_transaction() as conn:
        await conn.execute_query(
            "ALTER TABLE product ADD COLUMN date_published DATETIME DEFAULT CURRENT_TIMESTAMP;"
        )'''
...
async def create_upload_file(file: UploadFile = File(...), user: user_pydantic = Depends(get_current_user)):
    ...
    # name.png into "name" , "png"
    extention = filename.split(".")[1]

    if extention not in ["png","jpg"]:
        return {"status": "error","detail":"File extention not permitted"}

    # /static/images/secrettoken.png
    ...
    business = await Business.get(owner = user)
    owner = await business.owner # check if it is the owner
    ...
    if owner == user:
        ...
        raise HTTPException(
            status_code = status.HTTP_401_UNAUTHORIZED,
            detail = "not authenticated to perform this",
            headers = {"WWW-Autentication": "Bearer"}
        )
...
async def add_new_product(product: product_pydanticIn, 
    ...
    business = await Business.get(id = user.id)
    if not business:
        return {"status" : "not ok, error"}
...
async def update_product(id: int, update_info: product_pydanticIn, user: user_pydantic = Depends(get_current_user)):
    ...
    if user == owner and update_info["original_price"] > 0:
        ...
        raise HTTPException(
            status_code = status.HTTP_401_UNAUTHORIZED,
            detail = "not authenticated to perform this or invalid user input",
            headers = {"WWW-Autentication": "Bearer"}
        )
